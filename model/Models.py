...
def model_graph(features, params):
    ...
    img = tf.cast(tf.reshape(img, (512,512,3)),tf.float32)
    TR = tf.cast(tf.reshape(TR, (512,512,1)),tf.float32)
    TCL = tf.cast(tf.reshape(TCL, (512,512,1)),tf.float32)
    radius = tf.cast(tf.reshape(radius, (512,512,1)),tf.float32)
    cos_theta = tf.cast(tf.reshape(cos_theta, (512,512,1)),tf.float32)
    sin_theta = tf.cast(tf.reshape(sin_theta, (512,512,1)),tf.float32)
