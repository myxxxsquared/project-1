...
class BaseNet(object):
    ...
    def undo_minus_mean(self,image, MeanPixel):
        # this step in carried out in VGG
        return image + MeanPixel
    ...
    def _ConvLayer(self,inputs, filters=None, bias=None, shape=None, strides=(1,1,1,1), padding="SAME", name="Conv",trainable=True,initializer=xavier):
        """
        :param inputs:
        :param filters:
        :param bias:
    ...
    def _Pooling(self,inputs, pooling, padding="SAME", name="Pooling"):
    ...
    ##TODO upsampling: bilinear & deconv
    def _upsampling(self,inputs,upsampling='DeConv',**kwargs):
        """
        :param inputs:
...
class VGG16(BaseNet):
    def net_loading(self, **kwargs):
        # ConvNet.net_loading(ConvNet_weights, input_image_c_s, params["pooling"])
        """
        ...
        for i, name in enumerate(self.layer):
            Layer_type = name[:4]
            if Layer_type == 'conv':
                activations = self._ConvLayer(activations, shape=filter_shapes[name], padding=self.padding, name=name)
            elif Layer_type == 'relu':
                activations = tf.nn.relu(activations)
            elif Layer_type == 'pool':
                activations = self._Pooling(activations, self.pooling, name=name)
...
class ResNet(BaseNet):
    def net_loading(self,**kwargs):
        ...
        if not layer in ResNetSchedule.layouts: raise ValueError('no such resnet layout')
        ...
        activations = self._ConvLayer(activations, shape=(7,7,3,64), padding=self.pooling, strides=(1,2,2,1), name='stage1')
        ...
        for i,stage in enumerate(['stage2','stage3','stage4','stage5']):
            ...
            for block in range(structure[1]):
                if block==0 and stage=='stage2':
                    block_acti=self._Pooling(activations, 'max', name='Stage2_pooling')
                else:
                ...
                for step,filter_shape in enumerate(structure[0]):#filter_shape (k1,k2,output_channel)
                    ...
                    block_acti=self._ConvLayer(block_acti, shape=(
                        filter_shape[0], filter_shape[1], shape[-1], filter_shape[2]),
                                                     padding=self.pooling, strides=strides,
                                                     name='stage_' + str(block + 2)+'step_' + str(step))
                    if step==len(structure[0])-1: break
                    block_acti=tf.nn.relu(block_acti)
                shape_acti=activations.shape.as_list()
                shape_block=block_acti.shape.as_list()

                if shape_acti==shape_block:
                    activations=block_acti+activations
                else:
                    activations=block_acti+tf.concat([self._Pooling(activations,
                                                                    pooling='avg',
                                                                    name=stage+'_block_'+str(block)+'_pooling'),
                                                      tf.constant(tf.zeros(dtype=tf.float32,
                                                                           shape=(shape_block[0],
                                                                                  shape_block[1],
                                                                                  shape_block[2],
                                                                                  shape_block[3]-shape_acti[3])))],axis=-1)
                activations=tf.nn.relu(activations)
